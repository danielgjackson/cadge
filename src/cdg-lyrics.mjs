        ...
        if (analyzerResult.stepResult.parseResult) {
            ...
            for (const change of analyzerResult.changes) {
                ...
                if (change.action == 'group-add') {
                    ...
                    this.activeLines[change.groupId].timings = [];
                ...
                if (updateTimingPosition != null) {
                    if (this.activeLines[change.groupId].words) {
                        ...
                        for (let i = this.activeLines[change.groupId].timings.length; i <= this.activeLines[change.groupId].words.length; i++) { // <= to handle end of last word
                            const word = i > 0 ? this.activeLines[change.groupId].words[i - 1] : null;
                            // First word
                            if (word == null) {
                                // First word has not yet started, do not add timestamp yet
                                if (this.activeLines[change.groupId].words.length > 0 && updateTimingPosition < this.activeLines[change.groupId].words[0].left) {
                                    break;
                                }
                                // First word has started, add timestamp
                                this.activeLines[change.groupId].timings.push(time);
                            } else {    // non-first word
        ...
        for (const line of this.lines) {
            ...
            if (line.words && line.words.length > 0) {
                for (let i = 0; i <= line.words.length; i++) {
                    if (i < line.timings.length) {
                        lineParts.push('<' + formatTime(line.timings[i]) + '>');
                    }
